...
export default function ProjectCard({ project, viewerRole }: ProjectCardProps) {
  ...
  const getStatusLabel = (status: ActiveProject["status"]) => {
    if (status === "In Progress") return t.activeProjects.inProgress;
    if (status === "In Review") return t.activeProjects.review;
    if (status === "Completed") return t.activeProjects.completed;
    return status;
  };

  const counterpartLabel =
    project.counterpartLabel === "UMKM"
      ? t.activeProjects.card.counterpartUmkm
      : t.activeProjects.card.counterpartTalent;

  const clientName =
    project.clientName === "Talent belum dipilih"
      ? t.activeProjects.card.unassignedTalent
      : project.clientName;
    ...
    <div className="rounded-xl border border-hairline bg-card p-6">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-6">
          <div>
            <p className="font-body text-xs text-ink-muted">{t.activeProjects.card.budget}</p>
            <p className="mt-0.5 font-display text-sm font-black text-ink">
              {project.budgetLabel}
            </p>
          </div>
          <div>
            <p className="font-body text-xs text-ink-muted">{t.activeProjects.card.deadline}</p>
            <p className="mt-0.5 font-body text-base font-bold text-brand">
              {project.deadlineLabel}
            </p>
          </div>
          <div>
            <p className="font-body text-xs text-ink-muted">{t.activeProjects.card.lastUpdate}</p>
        ...
        {viewerRole !== "STUDENT" && (
          <div className="rounded-lg bg-canvas px-4 py-3 text-right">
            <p className="font-body text-xs text-ink-muted">{t.activeProjects.card.proposalsReceived}</p>
            <p className="mt-0.5 font-display text-sm font-black text-ink">
              {project.proposalCount ?? 0} {t.activeProjects.card.proposalsCount}
            </p>
          </div>
        )}
      </div>
      {project.paymentStatus === "HELD" && (
        <p className="mt-4 rounded-lg bg-success/10 px-4 py-3 text-xs font-semibold text-success">
          {t.activeProjects.card.paymentSecured}
        </p>
      )}
