...
export default function TwoFactorAuthCard({ enabled }: { enabled: boolean }) {
  ...
  function begin() {
    startTransition(async () => {
      const result = await beginTwoFactorSetupAction(password);
  ...
  function confirm() {
    startTransition(async () => {
      ...
      await Swal.fire({
        ...
        title: "2FA aktif",
        html: `
          <p>Simpan kode pemulihan ini di tempat aman:</p>
          <pre style="white-space:pre-wrap;margin-top:12px">${result.recoveryCodes.join("\n")}</pre>
        `,
  ...
  return (
    <section className="rounded-xl border border-[#ECECEC] dark:border-gray bg-white p-6 dark:bg-card">
