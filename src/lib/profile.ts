...
export const getCachedProfileData = cache(async () => {
    ...
    try {
      ...
      if (user) {
        ...
        return {
          ...
          addressDetail: student?.alamat_detail || user.umkm?.alamat_detail || "",
          provinceCode: student?.provinsi_kode || user.umkm?.provinsi_kode || "",
          provinceName: student?.provinsi_nama || user.umkm?.provinsi_nama || "",
          regencyCode: student?.kabupaten_kode || user.umkm?.kabupaten_kode || "",
          regencyName: student?.kabupaten_nama || user.umkm?.kabupaten_nama || "",
          districtCode: student?.kecamatan_kode || user.umkm?.kecamatan_kode || "",
          districtName: student?.kecamatan_nama || user.umkm?.kecamatan_nama || "",
          villageCode: student?.kelurahan_kode || user.umkm?.kelurahan_kode || "",
          villageName: student?.kelurahan_nama || user.umkm?.kelurahan_nama || "",
          about: user.bio || "Deskripsi profil belum diisi.",
          skills: student?.skills.map(({ skill }) => skill.name) || [],
          skillLevels: Object.fromEntries(student?.skills.map(({ skill, level }) => [skill.name, level]) || []),
          expectedBudgetMin: student?.expectedBudgetMin ?? null,
          expectedBudgetMax: student?.expectedBudgetMax ?? null,
  ...
  return {
    ...
    expectedBudgetMin: null as number | null,
    expectedBudgetMax: null as number | null,
